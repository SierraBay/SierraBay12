...
if (window.addEventListener) {
    window.addEventListener('load', function() {
        var table = document.getElementById('tele_table');
        if (table) {
...
window.filterZLevel = function(z) {
    window.selectedZ = z;
    ...
    var activeBtn = document.getElementById('btn_z_' + z);
    if (activeBtn) {
...
window.filterTable = function() {
    try {
        ...
        var selectedZ = window.selectedZ || 'all';
        
        var allTrs = document.getElementsByTagName('tr');
        for (var i = 0; i < allTrs.length; i++) {
            var row = allTrs.item(i);
            if (row && row.className && row.className.indexOf('tele-row') > -1) {
                var text = row.innerText || row.textContent || "";
                var zVal = row.getAttribute('data-z') || "";
                
                var matchesSearch = text.toLowerCase().indexOf(filter) > -1;
                var matchesZ = (selectedZ === 'all' || zVal === selectedZ);
                
                if (matchesSearch && matchesZ) {
                    row.style.display = "";
                } else {
...
window.sortTable = function(n) {
    try {
        ...
        rowsArray.sort(function(a, b) {
            ...
            var valA = cellA ? (cellA.innerText || cellA.textContent || "").replace(/^\s+|\s+$/g, '') : "";
            var valB = cellB ? (cellB.innerText || cellB.textContent || "").replace(/^\s+|\s+$/g, '') : "";
            
            var numA = parseFloat(valA);
            var numB = parseFloat(valB);
            if (!isNaN(numA) && !isNaN(numB)) {
                if (numA < numB) return nextDir === 'asc' ? -1 : 1;
                if (numA > numB) return nextDir === 'asc' ? 1 : -1;
                return 0;
            }
            
            var A = valA.toUpperCase();
            var B = valB.toUpperCase();
