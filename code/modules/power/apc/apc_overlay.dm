...
/obj/machinery/power/apc/on_update_icon()
	...
	if(update & 2)
		...
		if(!MACHINE_IS_BROKEN(src) && !GET_FLAGS(stat, MACHINE_STAT_MAINT) && update_state & UPDATE_ALLGOOD)
			...
			if(operating)
				AddOverlays(status_overlays_equipment[equipment_channel+2])
				AddOverlays(status_overlays_equipment[1])
				AddOverlays(status_overlays_lighting[lighting_channel+2])
				AddOverlays(status_overlays_lighting[1])
				AddOverlays(status_overlays_environ[environment_channel+2])
...
/obj/machinery/power/apc/proc/check_updates()
	...
	if(update_state & UPDATE_ALLGOOD)
		...
		update_overlay_chan["Equipment"] = equipment_channel
		update_overlay_chan["Lighting"] = lighting_channel
		update_overlay_chan["Enviroment"] = environment_channel
