	...
	if(href_list["action"])
		if(href_list["action"] == "enable")
			if(!active && HasFuel() && !IsBroken())
				active = 1
				update_icon()
				log_and_message_admins("has turned on [src.name] at [loc].", usr)
		if(href_list["action"] == "disable")
			if (active)
				active = 0
				update_icon()
				log_and_message_admins("has turned off [src.name] at [loc].", usr)
...
/obj/machinery/power/port_gen/pacman/super/explode()
	...
	log_and_message_admins("[src.name] exploded at [loc]!")
