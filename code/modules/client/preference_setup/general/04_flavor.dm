...
/datum/category_item/player_setup_item/physical/flavor/content(mob/user)
	. += "<b>Описание:</b><br>"
	. += "<a href='?src=\ref[src];flavor_text=open'>Изменить текст описания</a><br/>"
	. += "<a href='?src=\ref[src];flavour_text_robot=open'>Изменить текст описания робота</a><br/>"
...
/datum/category_item/player_setup_item/physical/flavor/OnTopic(href,list/href_list, mob/user)
	if(href_list["flavor_text"])
		switch(href_list["flavor_text"])
			if("open")
			if("general")
				var/msg = sanitize(input(usr,"Дайте общее описание вашего персонажа. Это будет видно независимо от одежды. Не включайте сюда информацию OOC.","Текст описания",html_decode(pref.flavor_texts[href_list["flavor_text"]])) as message, extra = 0)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
			else
				var/msg = sanitize(input(usr,"Впишите описание для [href_list["flavor_text"]].","Текст описания",html_decode(pref.flavor_texts[href_list["flavor_text"]])) as message, extra = 0)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
		SetFlavorText(user)
		...
		switch(href_list["flavour_text_robot"])
			if("open")
			if("Default")
				var/msg = sanitize(input(usr,"Впишите описание для вашего робота. Оно будет использоваться для любого модуля без индивидуальной настройки.","Текст описания",html_decode(pref.flavour_texts_robot["Default"])) as message, extra = 0)
				if(CanUseTopic(user))
					pref.flavour_texts_robot[href_list["flavour_text_robot"]] = msg
			else
				var/msg = sanitize(input(usr,"Впишите описание модуля [href_list["flavour_text_robot"]] у вашего робота. Если вы оставите это поле пустым, для этого модуля будет использоваться текст по умолчанию.","Текст описания",html_decode(pref.flavour_texts_robot[href_list["flavour_text_robot"]])) as message, extra = 0)
				if(CanUseTopic(user))
					pref.flavour_texts_robot[href_list["flavour_text_robot"]] = msg
...
/datum/category_item/player_setup_item/physical/flavor/proc/SetFlavorText(mob/user)
	var/HTML = "<body>"
	HTML += "<tt><center>"
	HTML += "<b>Изменение текста описания</b> <hr />"
	HTML += "<br></center>"
...
/datum/category_item/player_setup_item/physical/flavor/proc/SetFlavourTextRobot(mob/user)
	var/HTML = "<body>"
	HTML += "<tt><center>"
	HTML += "<b>Изменение текста описания робота</b> <hr />"
	HTML += "<br></center>"
