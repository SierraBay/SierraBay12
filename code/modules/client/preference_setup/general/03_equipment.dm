...
/datum/category_item/player_setup_item/physical/equipment/content()
	...
	. += "<br>"
	. += "Стандартная настройка датчика жизнедеятельности: <a href='?src=\ref[src];change_sensor_setting=1'>[pref.sensor_setting]</a><br />"
	. += "Блокировка датчика жизнедеятельности: <a href='?src=\ref[src];toggle_sensors_locked=1'>[pref.sensors_locked ? "Заблакировано" : "Разблокировано"]</a><br />"
...
/datum/category_item/player_setup_item/physical/equipment/OnTopic(href,list/href_list, mob/user)
	if(href_list["change_underwear"])
		...
		var/datum/category_item/underwear/selected_underwear = input(user, "Выберите нижнее бельё вашему персонажу:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.all_underwear[UWC.name]) as null|anything in UWC.items
		...
		var/new_backpack = input(user, "Выберите тип рюкзака:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.backpack) as null|anything in backpacks_by_name
		...
		var/switchMode = input("Выберите режим работы датчика жизнедеятельности:", "Датчик жизнедеятельности", pref.sensor_setting) as null | anything in SUIT_SENSOR_MODES
