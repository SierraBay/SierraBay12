...
/obj/machinery/appliance/cooker/microwave/Initialize()
	...
	for(var/newtime = 1 to 6)
		var/image/disp_image = image('icons/screen/radial.dmi', "radial_time")
...
/obj/machinery/appliance/cooker/microwave/use_tool(obj/item/O, mob/living/user, list/click_params)
	if (broken > 0)
...
/obj/machinery/appliance/cooker/microwave/RefreshParts()
	...
	heating_power = initial(heating_power) + laser_rating * 150
...
/obj/machinery/appliance/cooker/microwave/on_update_icon()
	..()
	if(dirtiness == 100)
...
/obj/machinery/appliance/cooker/microwave/examine(mob/user, distance)
	. = ..()
	if(broken > 0)
		to_chat(user, SPAN_WARNING("This microwave is very broken. You'll need to fix it before you can use it again."))
	else if (dirtiness == 100)
		to_chat(user, SPAN_WARNING("This microwave is covered in muck. You'll need to wipe it down or clean it out before you can use it again."))
