...
#define evac_chair(varName) var/obj/structure/bed/chair/shuttle/##varName	// [SIERRA-ADD]
#define ISEVAC_STARSHIP_FAST_CONTROLER istype(evacuation_controller, /datum/evacuation_controller/starship/fast)	// [SIERRA-ADD]
...
//  [SIERRA-ADD]
/obj/machinery/embedded_controller/radio/simple_docking_controller/escape_pod_berth/use_tool(obj/item/T, mob/living/carbon/human/user)
...
/obj/machinery/embedded_controller/radio/simple_docking_controller/escape_pod_berth/emag_act(remaining_charges, mob/user)
	if (!emagged)
		...
		if (istype(program, /datum/computer/file/embedded_program/docking/simple/escape_pod_berth))
			var/datum/computer/file/embedded_program/docking/simple/escape_pod_berth/P = program
			if (!P.armed)
				addtimer(new Callback(P, /datum/computer/file/embedded_program/docking/simple/escape_pod_berth/proc/arm), 5 MINUTES)	//  [SIERRA-ADD]
		return 1
