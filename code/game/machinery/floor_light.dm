...
/obj/machinery/floor_light/interface_interact(mob/user)
	...
	var/on = (power_state == POWER_USE_ACTIVE)
	change_power_mode(on ? POWER_USE_OFF : POWER_USE_ACTIVE)
...
/obj/machinery/floor_light/proc/update_brightness()
	if((power_state == POWER_USE_ACTIVE) && operable())
		if(light_range != default_light_range || light_power != default_light_power || light_color != default_light_colour)
			...
			set_power_consumption((light_range + light_power) * 20, POWER_USE_ACTIVE)
	else
