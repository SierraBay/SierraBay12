...
/datum/antag_contract/proc/ban_non_crew_antag()
	for(var/antag_type in list(MODE_WIZARD, MODE_RAIDER, MODE_VOXRAIDER, MODE_NINJA, MODE_MERCENARY, MODE_ACTOR, MODE_COMMANDO))
...
/datum/antag_contract/proc/complete(obj/item/device/uplink/close_uplink)
	...
	if(M)
		...
		if(M.current)
			to_chat(M.current, SPAN_NOTICE("Contract completed: [name] ([reward] TC). [pick("Nice work", "Good job", "Great job", "Well done", "Nicely done")], [M.current]."))
...
/datum/antag_contract/implant/generate_antag_reasons()
	..()
	reason_list[MODE_TRAITOR] = list("they want to make sure that the object is still interested in paying the cost of the uplink", 1.5, 25)
...
/datum/antag_contract/item/steal/proc/get_steal_reason(type)
	...
	var/reasonn
	if(!s_type)
		s_type = pick(CONTRACT_STEAL_MILITARY, CONTRACT_STEAL_OPERATION, CONTRACT_STEAL_SCIENCE)
	switch(s_type)
...
/datum/antag_contract/item/assassinate/complete(obj/item/device/uplink/close_uplink)
	...
	if((istype(_H) && !_H.stat && _H.mind) || (istype(_brain?.loc, /obj/item/device/mmi) && !target_detected_in_STD))
		if(M)
			to_chat(M, SPAN_WARNING("According to our information, the target ([target_real_name]) specified in the contract is still alive, don't try to deceive us or the consequences will be... Inevitable."))
		return
	if(!detected_less_tc)
		reward = reward * 2
	..(close_uplink)
