...
(function(){function q(){var d={"&":"&#38;","<":"&#60;",">":"&#62;",'"':"&#34;","'":"&#39;","/":"&#47;"},c=/&(?!#?\w+;)|<|>|"|'|\//g;return function(){return this?this.replace(c,function(e){return d[e]||e}):this}}function r(d,c,e){return("string"===typeof c?c:c.toString()).replace(d.define||m,function(l,f,k,a){0===f.indexOf("def.")&&(f=f.substring(4));f in e||(":"===k?(d.defineParams&&a.replace(d.defineParams,function(b,g,h){e[f]={arg:g,text:h}}),f in e||(e[f]=a)):(new Function("def","def['"+f+"']="+
a))(e));return""}).replace(d.use||m,function(l,f){d.useParams&&(f=f.replace(d.useParams,function(k,a,b,g){if(e[b]&&e[b].arg&&g)return k=(b+":"+g).replace(/'|\\/g,"_"),e.__exp=e.__exp||{},e.__exp[k]=e[b].text.replace(new RegExp("(^|[^\\w$])"+e[b].arg+"([^\\w$])","g"),"$1"+g+"$2"),a+"def.__exp['"+k+"']"}));return(l=(new Function("def","return "+f))(e))?r(d,l,e):l})}function n(d){return d.replace(/\\('|\\)/g,"$1").replace(/[\r\t\n]/g," ")}var p={version:"1.0.1-nanoui",templateSettings:{evaluate:/\{\{([\s\S]+?)\}\}/g,
interpolate:/\{\{:([\s\S]+?)\}\}/g,encode:/\{\{>([\s\S]+?)\}\}/g,use:/\{\{#([\s\S]+?)\}\}/g,define:/\{\{##\s*([\w\.$]+)\s*(:|=)([\s\S]+?)#\}\}/g,conditional:/\{\{\/?if\s*([\s\S]*?)\s*\}\}/g,conditionalElse:/\{\{else\s*([\s\S]*?)\s*\}\}/g,iterate:/\{\{\/?for\s*(?:\}\}|([\s\S]+?)\s*(?::\s*([\w$]+))?\s*(?::\s*([\w$]+))?\s*\}\})/g,props:/\{\{\/?props\s*(?:\}\}|([\s\S]+?)\s*(?::\s*([\w$]+))?\s*(?::\s*([\w$]+))?\s*\}\})/g,empty:/\{\{empty\}\}/g,varname:"data, config, helper",strip:!0,append:!0,selfcontained:!1},
template:void 0,compile:void 0};if("undefined"!==typeof module&&module.exports)module.exports=p;else if("function"===typeof define&&define.amd)define(function(){return p});else{var t=function(){return this||(0,eval)("this")}();t.doT=p}String.prototype.encodeHTML=q();var u={start:"'+(",end:")+'",endencode:"||'').toString().encodeHTML()+'"},v={start:"';out+=(",end:");out+='",endencode:"||'').toString().encodeHTML();out+='"},m=/$^/;p.template=function(d,c,e){c=c||p.templateSettings;var l=c.append?u:
v,f,k=0;d=c.use||c.define?r(c,d,e||{}):d;d=("var out='"+(c.strip?d.replace(/(^|\r|\n)\t* +| +\t*(\r|\n|$)/g," ").replace(/\r|\n|\t|\/\*[\s\S]*?\*\//g,""):d).replace(/'|\\/g,"\\$&").replace(c.interpolate||m,function(a,b){return l.start+n(b)+l.end}).replace(c.encode||m,function(a,b){f=!0;return l.start+n(b)+l.endencode}).replace(c.conditional||m,function(a,b){return b?"';if("+n(b)+"){out+='":"';}out+='"}).replace(c.conditionalElse||m,function(a,b){return b?"';}else if("+n(b)+"){out+='":"';}else{out+='"}).replace(c.iterate||
m,function(a,b,g,h){if(!b)return"';} } out+='";k+=1;g=g||"value";h=h||"index";b=n(b);a="arr"+k;return"';var "+a+"="+b+";if("+a+" && "+a+".length > 0){var "+g+","+h+"=-1,l"+k+"="+a+".length-1;while("+h+"<l"+k+"){"+g+"="+a+"["+h+"+=1];out+='"}).replace(c.props||m,function(a,b,g,h){if(!b)return"';} } out+='";k+=1;g=g||"value";h=h||"key";b=n(b);a="arr"+k;return"';var "+a+"="+b+";if("+a+" && Object.size("+a+") > 0){var "+g+";for( var "+h+" in "+a+"){ if (!"+a+".hasOwnProperty("+h+")) continue; "+g+"="+
a+"["+h+"];out+='"}).replace(c.empty||m,function(a){return"';}}else{if(true){out+='"}).replace(c.evaluate||m,function(a,b){return l.start+n(b)+l.end})+"';return out;").replace(/\n/g,"\\n").replace(/\t/g,"\\t").replace(/\r/g,"\\r").replace(/(\s|;|\}|^|\{)out\+='';/g,"$1").replace(/\+''/g,"").replace(/(\s|;|\}|^|\{)out\+=''\+/g,"$1out+=");f&&c.selfcontained&&(d="String.prototype.encodeHTML=("+q.toString()+"());"+d);try{return new Function(c.varname,d)}catch(a){throw"undefined"!==typeof console&&console.log("Could not create a template function: "+
d),a;}};p.compile=function(d,c){return p.template(d,null,c)}})();
