...
/mob/living/carbon/human
	var/list/health_controllers = list() // сюда складываем все контроллеры
...
/datum/asthma_controller/proc/RunLoop()
	while(running && owner)
		// --- Астма ---
		if(attack_active && world.time >= attack_next_tick_time)
			owner.adjustOxyLoss(rand(5, 8))
			sleep(10)
			if(prob(50)) owner.emote("gasp")
			attack_next_tick_time = world.time + 2 SECONDS
			if(world.time >= attack_end_time || owner.stat == DEAD || owner.chem_effects[CE_OXYGENATED])
				attack_active = FALSE
		...
		sleep(max(1, next_wake - world.time))
...
/datum/headache_controller/proc/RunLoop()
	while(running && owner)
		if(owner.stat == DEAD) break
...
/datum/hallucinations_controller/proc/RunLoop()
	while(running && owner)
		if(owner.stat == DEAD) break
...
/datum/seizures_controller/proc/RunLoop()
	while(running && owner)
		if(owner.stat == DEAD) break
