...
/obj/vehicles/proc/can_put_cargo(obj/object)
	...
	var/space_needed = BASE_STORAGE_CAPACITY(get_cargo_size(object))
	return space_needed <= (cargo_capacity - used_cargo_space)

/obj/vehicles/proc/get_cargo_size(obj/object)
...
/obj/vehicles/proc/put_cargo_item(mob/user, obj/O)
	...
	if(cargo_size > 4 && !do_after(user, 30, O))
		return FALSE
	user.visible_message("<span class = 'notice'>[user] loads [O] into [src].</span>")
	user.drop_from_inventory(O)
	O.loc = src
	cargo_contents += O
	used_cargo_space += BASE_STORAGE_CAPACITY(get_cargo_size(O))
	src.verbs |= /obj/vehicles/proc/get_cargo_item
	return TRUE

/obj/vehicles/proc/load_vehicle(obj/vehicles/v, mob/user)
...
/obj/vehicles/proc/eject_cargo_item(obj/object_removed, atom/movable/target)
	...
	used_cargo_space -= BASE_STORAGE_CAPACITY(get_cargo_size(object_removed))
	if(!cargo_contents.len)
		src.verbs -= /obj/vehicles/proc/get_cargo_item

/obj/vehicles/proc/handle_grab_attack(obj/item/grab/I, mob/user)
