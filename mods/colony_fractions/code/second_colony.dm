...
/obj/overmap/visitable/sector/facolony
	...
	Initialize(mapload)
		var/turf/unsimulated/map/keep = loc
		. = ..()
		if (keep && loc != keep)
			forceMove(keep)
		return .

	proc/is_sensor_visible(var/obj/overmap/ship/scanner)
		return FALSE

	proc/get_sensor_visibility(var/obj/overmap/ship/scanner)
		return 0
...
/datum/map_template/ruin/exoplanet/playablecolony2/after_load()
	...
	var/obj/overmap/visitable/sector/base_sector = map_sectors["[L.z]"]
	if (!istype(base_sector, /obj/overmap/visitable/sector))
		return
	var/turf/unsimulated/map/base_turf = get_turf(base_sector)

	var/obj/overmap/visitable/sector/exoplanet/planet = null
	for (var/obj/overmap/visitable/sector/exoplanet/P in base_turf)
		if (P.z == base_sector.z)
			planet = P
			break
	if (!planet)
		for (var/obj/overmap/visitable/sector/exoplanet/P in range(2, base_turf))
			if (P.z == base_sector.z)
				planet = P
				break
	if (!planet)
	...
	var/turf/unsimulated/map/pt = get_turf(planet)
	for (var/obj/overmap/visitable/sector/facolony/E in pt)
		qdel(E)

	var/obj/overmap/visitable/sector/facolony/sec = new(pt)
	sec.plane = planet.plane
	sec.layer = planet.layer + 1
