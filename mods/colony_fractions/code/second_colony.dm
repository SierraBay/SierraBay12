...
/datum/map_template/ruin/exoplanet/playablecolony2/proc/colony_inform()
	...
	if(GLOB.last_colony_type == "НАНОТРЕЙЗЕН")
		message += "<center><img src = ntlogo.png /><br />[FONT_LARGE("<b>NSV Sierra</b> Colony detected:")]</center><br>"
		message += "<center>Сенсорами объекта ИСН Сьерра зафиксирована посадка колониального судна корпорации NanoTrasen. Далее будут стандартные протоколы с взаимодействием между объектом и судном.</center>"
		message += "● Разрешены научные сделки на технологии или же обмен на артефактические структуры, Геологическая разведка планеты."
		message += "⊛ Разрешается любой обмен на интересующихся ресурс."
		message += "◍ Сотрудникам колонии одобрен проход на территорию объекта в случае чрезвычайное происшествия."
		message += "<center> Судно это актив корпорации Нанотрейзен, сделайте всё от себя зависящее не в вред основной миссии для помощи будущей колонии.</center>"
