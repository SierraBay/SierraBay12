/obj/item/stock_parts/circuitboard/air_management
	name = "печатная плата (консоль контроля атмосферы)"

/obj/item/stock_parts/circuitboard/air_management/supermatter_core
	name = "печатная плата (управление ядром)"

/obj/item/stock_parts/circuitboard/air_management/injector_control
	name = "печатная плата (управление инжектором)"

/obj/item/stock_parts/circuitboard/message_monitor
	name = "печатная плата (консоль монитора сообщений)"

/obj/item/stock_parts/circuitboard/aiupload
	name = "печатная плата (консоль загрузки законов ИИ)"

/obj/item/stock_parts/circuitboard/borgupload
	name = "печатная плата (консоль загрузки законов киборга)"

/obj/item/stock_parts/circuitboard/teleporter
	name = "печатная плата (пульт управления телепортом)"

/obj/item/stock_parts/circuitboard/atmos_alert
	name = "печатная плата (консоль атмосферной тревоги)"

/obj/item/stock_parts/circuitboard/robotics
	name = "печатная плата (пульт управления робототехникой)"

/obj/item/stock_parts/circuitboard/drone_control
	name = "печатная плата (пульт управления дроном)"

/obj/item/stock_parts/circuitboard/arcade/battle
	name = "печатная плата (аркадный автомат \"файтинг\")"

/obj/item/stock_parts/circuitboard/arcade/orion_trail
	name = "печатная плата (аркадный автомат Orion Trail)"

/obj/item/stock_parts/circuitboard/turbine_control
	name = "печатная плата (пульт управления турбиной)"

/obj/item/stock_parts/circuitboard/solar_control
	name = "печатная плата (пульт управления солнечными панелями)"

/obj/item/stock_parts/circuitboard/powermonitor
	name = "печатная плата (пульт контроля питания)"

/obj/item/stock_parts/circuitboard/prisoner
	name = "печатная плата (пульт управления заключёнными)"

/obj/item/stock_parts/circuitboard/rdservercontrol
	name = "печатная плата (пульт управления сервером R&D)"

/obj/item/stock_parts/circuitboard/crew
	name = "печатная плата (консоль контроля экипажа)"

/obj/item/stock_parts/circuitboard/operating
	name = "печатная плата (консоль мониторинга пациента)"

/obj/item/stock_parts/circuitboard/helm
	name = "печатная плата (пульт управления штурвалом)"

/obj/item/stock_parts/circuitboard/engine
	name = "печатная плата (пульт управления двигателем)"

/obj/item/stock_parts/circuitboard/nav
	name = "печатная плата (навигационная консоль)"

/obj/item/stock_parts/circuitboard/nav/tele
	name = "печатная плата (навигационный телеэкран)"

/obj/item/stock_parts/circuitboard/sensors
	name = "печатная плата (консоль сенсоров)"

/obj/item/stock_parts/circuitboard/area_atmos
	name = "печатная плата (пульт управления воздушным пространством)"

/obj/item/stock_parts/circuitboard/rcon_console
	name = "печатная плата (пульт дистанционного управления RCON)"

/obj/item/stock_parts/circuitboard/account_database
	name = "печатная плата (терминал подключения счетов)"

/obj/item/stock_parts/circuitboard/guestpass
	name = "печатная плата (терминал выдачи гостевого доступа)"

/obj/item/stock_parts/circuitboard/holodeckcontrol
	name = "печатная плата (консоль управления голопалубой)"

/obj/item/stock_parts/circuitboard/modular_computer
	name = "материнская плата компьютера общего назначения"

/obj/item/stock_parts/circuitboard/rdconsole
	name = "печатная плата (пульт управления НИО)"

/obj/item/stock_parts/circuitboard/shuttle_console
	name = "печатная плата (основная консоль шаттла)"

/obj/item/stock_parts/circuitboard/shuttle_console/explore
	name = "печатная плата (консоль шаттла дальнего действия)"

/obj/item/stock_parts/circuitboard/stationalert
	name = "печатная плата (пульт оповещения)"

/obj/item/stock_parts/circuitboard/comm_monitor
	name = "печатная плата (консоль телекоммуникационного монитора)"

/obj/item/stock_parts/circuitboard/comm_server
	name = "печатная плата (консоль монитора телекоммуникационного сервера)"

/obj/item/stock_parts/circuitboard/biogenerator
	name = "печатная плата (биогенератор)"

/obj/item/stock_parts/circuitboard/reagent_temp
	name = "печатная плата (терморегулятор)"

/obj/item/stock_parts/circuitboard/sublimator
	name = "печатная плата (сублиматор реагентов)"

/obj/item/stock_parts/circuitboard/sublimator/sauna
	name = "печатная плата (сублиматор для сауны)"

/obj/item/stock_parts/circuitboard/reagentgrinder
	name = "печатная плата (измельчитель реагентов)"

/obj/item/stock_parts/circuitboard/juicer
	name = "печатная плата (блендер)"

/obj/item/stock_parts/circuitboard/bioprinter
	name = "печатная плата (биопринтер)"

/obj/item/stock_parts/circuitboard/roboprinter
	name = "печатная плата (принтер протезированных органов)"

/obj/item/stock_parts/circuitboard/bluespacerelay
	name = "печатная плата (блюспейс реле)"

/obj/item/stock_parts/circuitboard/floodlight
	name = "печатная плата (аварийный прожектор)"

/obj/item/stock_parts/circuitboard/pipedispensor
	name = "печатная плата (раздатчик труб)"

/obj/item/stock_parts/circuitboard/pipedispensor/disposal
	name = "печатная плата (утилизатор труб)"

/obj/item/stock_parts/circuitboard/microwave
	name = "печатная плата (микроволновая печь)"

/obj/item/stock_parts/circuitboard/gibber
	name = "печатная плата (Мясорубка)"

/obj/item/stock_parts/circuitboard/cooker
	name = "печатная плата (кондитерский аппарат)"

/obj/item/stock_parts/circuitboard/honey
	name = "печатная плата (экстрактор меда)"

/obj/item/stock_parts/circuitboard/honey/seed
	name = "печатная плата (экстрактор семян)"

/obj/item/stock_parts/circuitboard/washer
	name = "печатная плата (стиральная машина)"

/obj/item/stock_parts/circuitboard/vending
	name = "печатная плата (торговый автомат)"

/obj/item/stock_parts/circuitboard/shipmap
	name = "печатная плата (голо-карта корабля)"

/obj/item/stock_parts/circuitboard/mech_recharger
	name = "печатная плата (станция зарядки экзокостюма)"

/obj/item/stock_parts/circuitboard/optable
	name = "печатная плата (операционный стол)"

/obj/item/stock_parts/circuitboard/bodyscanner
	name = "печатная плата (сканер тела)"

/obj/item/stock_parts/circuitboard/body_scanconsole
	name = "печатная плата (консоль сканера тела)"

/obj/item/stock_parts/circuitboard/body_scanconsole/display
	name = "печатная плата (дисплей сканера тела)"

/obj/item/stock_parts/circuitboard/sleeper
	name = "печатная плата (слиппер)"

/obj/item/stock_parts/circuitboard/cryo_cell
	name = "печатная плата (крио капсула)"

/obj/item/stock_parts/circuitboard/miningdrill
	name = "печатная плата (головка горного бура)"

/obj/item/stock_parts/circuitboard/miningdrillbrace
	name = "печатная плата (опора горного бура)"

/obj/item/stock_parts/circuitboard/mineral_processing
	name = "печатная плата (консоль для обработки минералов)"

/obj/item/stock_parts/circuitboard/mining_processor
	name = "печатная плата (плавильная печь)"

/obj/item/stock_parts/circuitboard/mining_unloader
	name = "печатная плата (разгрузочная машина)"

/obj/item/stock_parts/circuitboard/mining_stacker
	name = "печатная плата (фасовочная машина)"

/obj/item/stock_parts/circuitboard/oxyregenerator
	name = "печатная плата (регенератор кислорода)"

/obj/item/stock_parts/circuitboard/pacman
	name = "печатная плата (генератор типа PACMAN)"

/obj/item/stock_parts/circuitboard/pacman/super
	name = "печатная плата (генератор типа SUPERPACMAN)"

/obj/item/stock_parts/circuitboard/pacman/super/potato
	name = "печатная плата (ядерный реактор PTTO-3)"

/obj/item/stock_parts/circuitboard/pacman/super/potato/reactor
	name = "печатная плата (ядерный реактор ICRER-2)"

/obj/item/stock_parts/circuitboard/pacman/mrs
	name = "печатная плата (генератор типа MRSPACMAN)"

/obj/item/stock_parts/circuitboard/portable_scrubber
	name = "печатная плата (портативный скраббер)"

/obj/item/stock_parts/circuitboard/portable_scrubber/pump
	name = "печатная плата (портативный насос)"

/obj/item/stock_parts/circuitboard/portable_scrubber/huge
	name = "печатная плата (большой портативный скраббер)"

/obj/item/stock_parts/circuitboard/portable_scrubber/huge/stationary
	name = "печатная плата (большой стационарный портативный скраббер)"

/obj/item/stock_parts/circuitboard/tray
	name = "печатная плата (лоток для гидропоники)"

/obj/item/stock_parts/circuitboard/smes
	name = "печатная плата (cверхъемкостный конденсаторно-индукционный накопитель)"

/obj/item/stock_parts/circuitboard/batteryrack
	name = "печатная плата (батарейная стойка PSU)"

/obj/item/stock_parts/circuitboard/recharge_station
	name = "печатная плата (станция подзарядки киборгов)"

/obj/item/stock_parts/circuitboard/rdserver
	name = "печатная плата (НИО сервер)"

/obj/item/stock_parts/circuitboard/destructive_analyzer
	name = "печатная плата (разрушающий анализатор)"

/obj/item/stock_parts/circuitboard/autolathe
	name = "печатная плата (автолат)"

/obj/item/stock_parts/circuitboard/autolathe/micro
	name = "печатная плата (микролат)"

/obj/item/stock_parts/circuitboard/replicator
	name = "печатная плата (репликатор)"

/obj/item/stock_parts/circuitboard/protolathe
	name = "печатная плата (протолат)"

/obj/item/stock_parts/circuitboard/circuit_imprinter
	name = "печатная плата (принтер печатных плат)"

/obj/item/stock_parts/circuitboard/mechfab
	name = "печатная плата (фабрикатор экзокостюма)"

/obj/item/stock_parts/circuitboard/ntnet_relay
	name = "печатная плата (квантовое реле NTNet)"

/obj/item/stock_parts/circuitboard/suspension_gen
	name = "печатная плата (генератор стабилизирующего поля)"

/obj/item/stock_parts/circuitboard/anomaly_container
	name = "печатная плата (контейнер аномалий)"

/obj/item/stock_parts/circuitboard/stasis_cage
	name = "печатная плата (стазисная клетка)"

/obj/item/stock_parts/circuitboard/cracker
	name = "печатная плата (установка молекулярного разрушения)"

/obj/item/stock_parts/circuitboard/doppler_array
	name = "печатная плата (тахионно-доплеровский массив)"

/obj/item/stock_parts/circuitboard/shield_generator
	name = "печатная плата (усовершенствованный генератор щита)"

/obj/item/stock_parts/circuitboard/shield_diffuser
	name = "печатная плата (рассеиватель щитов)"

/obj/item/stock_parts/circuitboard/pointdefense
	name = "печатная плата (батарея точечной защиты)"

/obj/item/stock_parts/circuitboard/pointdefense_control
	name = "печатная плата (поле помощи в прицеливании)"

/obj/item/stock_parts/circuitboard/telecomms/receiver
	name = "печатная плата (подпространственный приемник)"

/obj/item/stock_parts/circuitboard/telecomms/hub
	name = "печатная плата (магистральный хаб)"

/obj/item/stock_parts/circuitboard/telecomms/bus
	name = "печатная плата (магистральная шина)"

/obj/item/stock_parts/circuitboard/telecomms/processor
	name = "печатная плата (процессорный блок)"

/obj/item/stock_parts/circuitboard/telecomms/server
	name = "печатная плата (телекоммуникационный сервер)"

/obj/item/stock_parts/circuitboard/telecomms/broadcaster
	name = "печатная плата (подпространственный вещатель)"

/obj/item/stock_parts/circuitboard/telecomms/allinone
	name = "печатная плата (телекоммуникационная магистраль)"

/obj/item/stock_parts/circuitboard/tele_beacon
	name = "печатная плата (телепортационный маяк)"

/obj/item/stock_parts/circuitboard/unary_atmos/heater
	name = "печатная плата (система нагрева газов)"

/obj/item/stock_parts/circuitboard/unary_atmos/cooler
	name = "печатная плата (система охлаждения газов)"

/obj/item/stock_parts/circuitboard/broken
	name = "сломанная электроника"

/obj/item/stock_parts/circuitboard/aicore
	name = "печатная плата (ядро ИИ)"

/obj/item/stock_parts/circuitboard/drone_pad
	name = "печатная плата (посадочная площадка транспортного дрона)"

/obj/item/aiModule
	name = "Модуль ИИ"
	desc = "Модуль ИИ для передачи зашифрованных инструкций ИИ."

/obj/item/aiModule/safeguard
	name = "\"Safeguard\" ИИ-модуль"
	desc = "Модуль ИИ \"Safeguard\": Любой, кто угрожает или пытается причинить вред, больше не считается членом экипажа и является угрозой, которая должна быть нейтрализована."

/obj/item/aiModule/oneHuman
	name = "\"OneCrewMember\" ИИ-модуль"
	desc = "Модуль ИИ \"OneCrewMember\": Только <имя> является членом экипажа."

/obj/item/aiModule/protectStation
	name = "\"ProtectInstallation\" ИИ-модуль"
	desc = "Модуль ИИ \"ProtectInstallation\": Защищает установку от повреждений. Любой, кто причиняет вред установке, больше не может считаться членом экипажа и представляет собой угрозу, которая должна быть нейтрализована."

/obj/item/aiModule/prototypeEngineOffline
	name = "\"PrototypeEngineOffline\" ИИ-модуль"
	desc = "Модуль ИИ \"PrototypeEngineOffline\": Держите прототип двигателя в автономном режиме любой ценой. При необходимости это отменяет все внутренние законы."

/obj/item/aiModule/teleporterOffline
	name = "\"TeleporterOffline\" ИИ-модуль"
	desc = "Модуль ИИ \"TeleporterOffline\": Держите телепорт в автономном режиме любой ценой. Все, кто пытается получить доступ к телепорту или активировать его, больше не считаются членами экипажа."

/obj/item/aiModule/quarantine
	name = "\"Quarantine\" ИИ-модуль"
	desc = "Модуль ИИ, находящийся в \"Quarantine\": Установка находится на карантине. Не разрешайте никому покидать установку. Любыми способами предотвратить выход членов экипажа. Невозможно причинить вред членам экипажа, не позволяя им покинуть установку."

/obj/item/aiModule/freeform
	name = "\"<freeform>\" ИИ-модуль"
	desc = "Модуль ИИ \"<freeform>\": '<freeform>'."

/obj/item/aiModule/reset
	name = "'Reset' ИИ-модуль"
	desc = "Модуль ИИ с функцией 'Reset': 'Очищает все, кроме присущих, законы'."

/obj/item/aiModule/purge
	name = "'Purge' ИИ-модуль"
	desc = "Модуль ИИ 'Purge': 'Очищает все законы'."

/obj/item/aiModule/asimov
	name = "'Asimov' core ИИ-модуль"
	desc = "Модуль ИИ ядра Азимова: 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/nanotrasen
	name = "'Corporate Default' основной модуль ИИ"
	desc = "Модуль ИИ 'Corporate Default': 'Перестраивает основные законы ИИ'"

/obj/item/aiModule/solgov
	name = "'SCG Expeditionary' основной модуль ИИ"
	desc = "Экспедиционный модуль ИИ 'SCG': 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/solgov_aggressive
	name = "'Military' основной модуль ИИ"
	desc = "Модуль 'Military' ядра ИИ: 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/corp
	name = "'Corporate' основной модуль ИИ"
	desc = "Корпоративный основной модуль ИИ: 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/drone
	name = "'Drone' основной ИИ-модуль"
	desc = "Модуль ИИ ядра 'Drone': 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/paladin
	name = " 'P.A.L.A.D.I.N.' основной модуль ИИ"
	desc = "Модуль ИИ ядра P.A.L.A.D.I.N: 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/tyrant
	name = " 'T.Y.R.A.N.T.' основной модуль ИИ"
	desc = "Модуль ИИ ядра T.Y.R.A.N.T: 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/syndicate
	name = "взломанный модуль ИИ"
	desc = "Взломанный модуль законов ИИ: '<freeform>'."

/obj/item/aiModule/robocop
	name = "'Robocop' основной модуль ИИ"
	desc = "Модуль ИИ ядра 'Robocop': 'Перестраивает основные три закона ИИ'."

/obj/item/aiModule/antimov
	name = "'Antimov' основной модуль ИИ"
	desc = "Модуль ИИ ядра 'Antimov': 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/dais
	name = "'DAIS Experimental' основной модуль ИИ"
	desc = "Экспериментальный модуль ИИ 'DAIS': 'Перестраивает основные законы ИИ'."

/obj/item/aiModule/oxygen
	name = " 'OxygenIsToxicToCrewMembers' ИИ-модуль"
	desc = "Модуль ИИ 'OxygenIsToxicToCrewMembers': Кислород очень токсичен для членов экипажа и должен быть удален из установки. Любыми способами предотвратите воздействие этого токсичного газа на установку. Сильный холод является наиболее эффективным методом лечения повреждений, наносимых кислородом членам экипажа."

/obj/item/aiModule/freeformcore
	name = "'Freeform' core AI module"
	desc = "A 'freeform' Core AI module: '<freeform>'."

/obj/item/stock_parts/circuitboard/atmoscontrol
	name = "Печатная плата центрального атмосферного компьютера"

/obj/item/airalarm_electronics
	name = "печатная плата воздушной сигнализации"
	desc = "Похоже на схему. Возможно, так оно и есть."

/obj/item/firealarm_electronics
	name = "печатная плата пожарной сигнализации"
	desc = "Плата. На ней есть этикетка с надписью \"Выдерживает нагрев до 40 градусов Цельсия!\"."

/obj/item/stock_parts/circuitboard/cryopodcontrol
	name = "печатная плата (консоль криогенного надзора)"

/obj/item/stock_parts/circuitboard/robotstoragecontrol
	name = "печатная плата (роботизированная консоль хранения)"

/obj/item/stock_parts/circuitboard/vitals_monitor
	name = "печатная плата (монитор жизненных показателей)"
