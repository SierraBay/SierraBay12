...
/client/proc/play_screentext_on_client_screen(input_text = "AHTUNG RAZRAB DAUN сообщите разработчику", holding_on_screen_time = 5 SECONDS, need_output_every_word = TRUE, delay_between_words = 0.1 SECONDS, text_color = "#ff3333", input_shrift = "Verdana", clear_screen = TRUE)
	...
	// Сохраняем параметры стиля в объекте
	...
	if(need_output_every_word)
		...
		for(var/word in words_list)
			...
			T.maptext = {"<span style='vertical-align:top; text-align:center;
				color: [T.text_color]; font-size: 300%;
				text-shadow: 1px 1px 2px black, 0 0 1em black, 0 0 0.2em black;
				font-family: [T.shrift], "Pterra";'>[T.result_phrase]</span>"}
		...
		T.maptext = {"<span style='vertical-align:top; text-align:center;
			color: [T.text_color]; font-size: 300%;
			text-shadow: 1px 1px 2px black, 0 0 1em black, 0 0 0.2em black;
			font-family: [T.shrift], "Pterra";'>[input_text]</span>"}
...
/client/proc/smooth_delete_screentext_from_client_screen(obj/screen/screen_text_shit/input_screen, word_erasing = FALSE)
	if(!input_screen) //А экранчик то нам уже удалили
		return
	if(word_erasing)
		// Создаем копию текущего списка слов для удаления
		var/list/words_to_remove = input_screen.current_words_list.Copy()
		// Удаляем слова в обратном порядке
		for(var/i = length(words_to_remove), i >= 1, i--)
			if(input_screen.deleted_by_external) // Проверяем, не удалили ли экран извне
				break
			...
			input_screen.maptext = {"<span style='vertical-align:top; text-align:center;
				color: [input_screen.text_color]; font-size: 300%;
				text-shadow: 1px 1px 2px black, 0 0 1em black, 0 0 0.2em black;
				font-family: [input_screen.shrift], "Pterra";'>[input_screen.result_phrase]</span>"}
...
/client/proc/start_counting_back_on_screen(time = 10 SECONDS, text_color = "#ff3333", input_shrift = "Verdana")
	...
	T.shrift = input_shrift
	T.appearance_flags = APPEARANCE_UI_IGNORE_ALPHA
	T.alpha = 255  // Важно: сразу видимый
	...
	screen += T
	animate(T, alpha = 255, time = 5, easing = EASE_IN)  // Плавное появление
	...
	while(current_time >= 0)
		...
		T.maptext = {"<span style='vertical-align:top; text-align:center;
			...
			text-shadow: 1px 1px 2px black, 0 0 1em black, 0 0 0.2em black;
			font-family: [input_shrift], "Pterra";'>[mins_text]:[secs_text]</span>"}

		current_time--
		sleep(1 SECOND)

	if(!QDELETED(T) && screen)
		animate(T, alpha = 0, time = 5, easing = EASE_OUT)  // Плавное исчезновение
		sleep(5)
		screen -= T
		qdel(T)
