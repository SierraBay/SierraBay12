 ...
 * Use TYPE_PROC_REF(procname), TYPE_PROC_REF(type,procname) or GLOBAL_PROC_REF(procname) macros to validate the passed in proc at compile time.
 * TYPE_PROC_REF for procs defined on current type or it's ancestors, TYPE_PROC_REF for procs defined on unrelated type and GLOBAL_PROC_REF for global procs.
...
/datum/proc/UnregisterSignal(datum/target, sig_type_or_types)
	...
	if(!length(_signal_procs[target]))
