		...
		if((istype(mob.machine, /obj/machinery/computer/telecomms/traffic) && (mob.machine in view(1, mob))) || (issilicon(mob) && istype(mob.machine, /obj/machinery/computer/telecomms/traffic) ))
			...
			if(Machine.SelectedServer)
				...
				if(LAZYLEN(compileerrors))
					...
					send_output(src, "([LAZYLEN(compileerrors)] errors)", "tcserror")
					...
					for(var/mob/M in Machine.viewingcode)
						if(M.client)
							...
							send_output(M, "([LAZYLEN(compileerrors)] errors)", "tcserror")
		...
		if((istype(mob.machine, /obj/machinery/computer/telecomms/traffic) && (mob.machine in view(1, mob))) || (issilicon(mob) && istype(mob.machine, /obj/machinery/computer/telecomms/traffic) ))
			...
			if(Machine.SelectedServer)
				...
				if(LAZYLEN(compileerrors))
					...
					signal.data["message"] = ""
					if(LAZYLEN(Server.freq_listening) > 0)
