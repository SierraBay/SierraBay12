...
*/
/n_Parser/nS_Parser/proc/Precedence(node/expression/c_operator/top, node/expression/c_operator/input)
	if(istype(top))
		top=top.precedence
	if(istype(input))
...
/*
	...
	- <GetUnaryOperator()>
*/
/n_Parser/nS_Parser/proc/GetOperator(O, type=/node/expression/c_operator, L[])
	if(istype(O, type))
