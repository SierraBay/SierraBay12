...
/obj/structure/noose
	...
	icon_state = "noose"
	icon = 'mods/utility_items/icons/noose.dmi'
	anchored = TRUE
	can_buckle = TRUE
...
/obj/structure/noose/use_tool(obj/item/W, mob/user, params)
	if(W.edge)
...
/obj/structure/noose/user_buckle_mob(mob/living/carbon/human/M, mob/user)
	...
	if(M == user)
		M.visible_message(\
			SPAN_DANGER("[user] attempts to tie \the [src] over their own neck!"),\
			SPAN_DANGER("You attempt to tie \the [src] over your neck!"))

		if(do_after(user, 5 SECONDS))
			if(buckle_mob(M))
				M.visible_message(\
					SPAN_WARNING("[user] ties \the [src] over their own neck!"),\
					SPAN_WARNING("You tie \the [src] over your neck!"))
				playsound(user, 'mods/utility_items/sounds/noosed.ogg', 50, 1, -1)
				return TRUE

		user.visible_message(\
			SPAN_WARNING("[user] fails to tie \the [src] over their own neck!"),\
...
/obj/structure/noose/Process()
	...
	switch(ticks)
		...
		if(3)
			...
			if(buckled_mob)
				playsound(buckled_mob, 'mods/utility_items/sounds/noose_idle.ogg', 50, 1, -3)
