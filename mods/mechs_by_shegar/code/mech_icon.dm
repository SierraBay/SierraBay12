//TODO: Стоит сделать следующим образом: Добавить функцию update_passenger_layers() которая будет вызываться при повороте
//Меха. Тогда не придётся перерисовывать спрайт пассажиров каждый раз.
///Рисует пассажиров на мехе по новому.
...
/mob/living/exosuit/update_pilots(update_overlays = TRUE)
