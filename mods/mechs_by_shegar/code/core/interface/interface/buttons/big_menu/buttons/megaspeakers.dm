/obj/screen/exosuit/menu_button/megaspeakers
	...
	button_desc = "Позволяет громко и чётко говорить с помощью громкоговорителя меха. <br> -При нажатии ALT + ЛКм позволяет заранее записать фразу, -При нажатии ПКМ позволяет быстро проиграть записанную фразу. Работает и с быстрой кнопкой."
