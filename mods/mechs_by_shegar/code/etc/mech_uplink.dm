...
if(round_duration_in_ticks > 15 MINUTES)
	...
	if(GLOB.war_declared)
		...
		return
	to_chat(usr, SPAN_GOOD("Запрос одобрен, доп средства выделены."))	command_announcement.Announce("В секторе была замечена телепортация большого объема телекристаллов, использующихся Горлекскими Мародерами. Рекомендуется вызвать поддержку с ЦК для урегулирования ситуации.", "Показания датчиков [station_name()]" , msg_sanitized = 1, zlevels = GLOB.using_map.station_levels)
	GLOB.max_mech = 1
