...
/mob/observer/eye/landing/Initialize(mapload, shuttle_tag)
...
/mob/observer/eye/landing/setLoc(turf/T)
	T = get_turf(T)
...
/mob/observer/eye/landing/possess(mob/user)
	. = ..()
	if(owner && owner.client)
		owner.client.view = LANDING_VIEW
		owner.client.images += landing_images

/mob/observer/eye/landing/release(mob/user)
	if(owner && owner.client)
...
/mob/observer/eye/possess(mob/user)
	if(owner && owner != user)
		return
...
/mob/observer/eye/release(mob/user)
	if(owner != user || !user)
		return
	if(owner.eyeobj != src)
		return
