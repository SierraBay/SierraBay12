GLOBAL_LIST_INIT(hive_data_bool)
	hive_data_bool = list(
		"maximum_existing_mobs"			= FALSE, // Exact number set separately
		"spread_on_lower_z_level"		= FALSE, // Spread via wires "falling down" from higher z-level
...
GLOBAL_LIST_INIT(hive_data_float)
 ...
 // prob() of hive node leaving hive-themed oddity on death // "core_oddity_drop_chance"		= 50

GLOBAL_LIST_INIT(hive_names)
	hive_names = list("Von Neumann", "Lazarus", "Abattoir", "Auto-Surgeon", "NanoTrasen",
		"NanoNurse", "Vivisector", "Ex Costa", "Apostasy", "Gnosis", "Balaam", "Ophite",
		"Sarif", "VersaLife", "Slylandro", "SHODAN", "Pandora", "Fisto")

GLOBAL_LIST_INIT(hive_surnames)
	hive_surnames = list("Mk I", "Mk II", "Mk III", "Mk IV", "Mk V",
...
GLOBAL_VAR_INIT(hivemind_panel)
	hivemind_panel = new /datum/hivemind_panel
...
//		GLOB.hive_data_float["core_oddity_drop_chance"] = CLAMP(percent ? percent : 0, 0, 100)
	...
	if(href_list["spawn_hive"])
		...
		if(wrong_place_to_be_in)
			message_admins("Hivemind spawned at \the [get_area(wrong_place_to_be_in)] by [usr.ckey]")
	...
	if(href_list["really_kill_hive"])
		...
		for(var/obj/machinery/hivemind_machine/i in world)
			i.destruct()
		for(var/obj/wireweed/i in world)
			i.Destroy()
