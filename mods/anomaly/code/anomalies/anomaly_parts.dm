...
/obj/anomaly/Initialize()
	...
	if(multitile)
		...
		else if(multititle_parts_range == "2x2")
			var/turf/REF = get_turf(src)
			var/obj/anomaly/part/A = new(get_step(REF, WEST))
			var/obj/anomaly/part/B = new(get_step(REF, NORTHWEST))
			var/obj/anomaly/part/C = new(get_step(REF, NORTH))
			list_of_parts = list(A,B,C)
