...
/obj/anomaly/rvach/stop_processing_long_effect()
	addtimer(new Callback(src, PROC_REF(rvach_gib)), 3 SECONDS)
	.=..()
...
/proc/rvach_pull_around(atom/target, pull_range = 255, pull_power = STAGE_FIVE)
	for(var/atom/A in range(pull_range, target))
		if(ismech(A))
			continue
		A.rvach_anomaly_pull(target, pull_power)


/atom/proc/rvach_anomaly_pull(S, current_size)
	return

/obj/item/rvach_anomaly_pull(S, current_size)
	step_towards(src, S)

/mob/living/rvach_anomaly_pull(S, current_size)
	step_towards(src, S)
	Weaken(5)
