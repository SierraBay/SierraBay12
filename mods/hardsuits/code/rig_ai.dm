...
/obj/item/rig_module/simple_ai/Initialize()
	. = ..()
	...
	energy_warnings = list(
		new /datum/rig_warning(0.5, "Warning: hardsuit power level below 50%",         'mods/hardsuits/sound/shortbeep.ogg'),
		new /datum/rig_warning(0.3, "Warning: hardsuit power level below 30%",         'mods/hardsuits/sound/beep.ogg'),
		new /datum/rig_warning(0.1, "Warning: hardsuit power level is critically low", 'mods/hardsuits/sound/loudbeep.ogg'),
		)
...
/obj/item/rig_module/simple_ai/Process()
	...
	if(H.stat == DEAD && saved_stat != DEAD)
		playsound(H, 'mods/hardsuits/sound/dead.ogg', 50)
	saved_stat = H.stat
...
/obj/item/rig_module/simple_ai/proc/process_warnings(mob/living/carbon/human/H)
	var/current_health = 100 - (H.getOxyLoss() + H.getBruteLoss() + H.getFireLoss())
	...
	var/current_rig_damage = holder.chest.damage
...
/obj/item/rig_module/simple_ai/proc/handle_module_damage(source, obj/item/rig_module/dam_module)
	if(dam_module == src) // ai module is damaged, we need to forget about old messages and show to the user that we broke
		...
		if(dam_module.damage >= MODULE_DESTROYED)
			rig_message(destroyed_message, message_class = "danger", message_type = "[dam_module.name]", sound = 'mods/hardsuits/sound/longbeep.ogg')
		else
			rig_message(damage_message, message_class = "warning", message_type = "[dam_module.name]", sound = 'mods/hardsuits/sound/beep.ogg')
			restart_cooldown = 10
		rig_messages_process(holder.wearer)
		return

	if(dam_module.damage >= MODULE_DESTROYED)
		rig_message("The [source] has disabled your [dam_module.interface_name]!", message_class = "warning", message_type = "[dam_module.name]", sound = 'mods/hardsuits/sound/longbeep.ogg')
	else
		rig_message("The [source] has damaged your [dam_module.interface_name]!", message_class = "warning", message_type = "[dam_module.name]", sound = 'mods/hardsuits/sound/beep.ogg')
...
/obj/item/rig_module/simple_ai/advanced/Initialize()
	. = ..()
	get_random_voice()
...
/obj/item/rig_module/simple_ai/advanced/on_health(mob/living/carbon/human/H)
	...
	if(H.getOxyLoss() > 40)
		if(try_inject(H, chem_disp, list("dexalin plus", "dexalin", "tricordrazine")))
			return
	if(H.getFireLoss() > 40)
	...
	if(H.getBruteLoss() > 40)
		if(try_inject(H, chem_disp, list("bicaridine", "tricordrazine")))
			return
	if(H.get_shock() > 40 || H.shock_stage > 40)
...
/obj/item/rig_module/simple_ai/advanced/proc/try_inject(mob/living/carbon/human/H, obj/item/rig_module/chem_dispenser/chem_disp, list/reagents)
	...
	for(var/reagent in reagents)
		...
		if(H.reagents.get_reagent_amount(chem_disp.charges[reagent]) > 1)
			continue
