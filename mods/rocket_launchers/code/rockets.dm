				...
				switch(def_zone)
					if(BP_HEAD , BP_CHEST, BP_MOUTH, BP_EYES)
						for(var/mob/living/pilot in mech.pilots)
							pilot.apply_damage(damage, DAMAGE_BURN)
							to_chat(pilot, SPAN_WARNING("You feel a wave of heat wash over you!"))
							pilot.adjust_fire_stacks(rand(4,6) * iterator / 2)
							pilot.IgniteMob()

				mech.apply_damage(damage * 4, DAMAGE_BURN, def_zone)

		else if(isliving(A))
			var/mob/living/M = A
			M.apply_damage(damage, DAMAGE_BURN, def_zone)
...
/obj/item/projectile/bullet/rpg_rocket/heat/on_hit(atom/target, blocked, def_zone)
	...
	if(ismech(target))
		...
		switch(def_zone)
			if(BP_HEAD , BP_CHEST, BP_MOUTH, BP_EYES)
				for(var/mob/living/pilot in mech.pilots)
					pilot.apply_damage(heat_damage, DAMAGE_BURN)
					to_chat(pilot, SPAN_WARNING("You feel a wave of heat wash over you!"))
		...
		M.apply_damage(heat_damage, DAMAGE_BURN, def_zone)
		to_chat(M, SPAN_WARNING("You feel a wave of heat wash over you!"))
