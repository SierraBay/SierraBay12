...
/obj/structure/mortar
	name = "Mortar"
	desc = "Mortar"
	icon = 'mods/mortar/icons/mortar.dmi'
...
/obj/structure/mortar/attackby(obj/item/O, mob/user)
	...
	if(istype(O, /obj/item/mortar_shell))
		...
		playsound(loc, 'mods/mortar/sounds/rpgreload.ogg', 50, 1)
		busy = 1
		...
		if(do_after(user, 15, src))
			...
			playsound(loc, 'mods/mortar/sounds/mortar_fire.ogg', 50, 1)
			busy = 0
			firing = 1
			...
			spawn(travel_time) //What goes up
				playsound(T, 'mods/mortar/sounds/mortar_falling.ogg', 50, 1)
