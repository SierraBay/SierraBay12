...
exactly 120 "to_world uses" '\sto_world\('
...
exactly 321 "spawn uses" '^\s*spawn\s*\(\s*(-\s*)?\d*\s*\)' -P
...
exactly 14 "uses of .len" '\.len\b' -P
exactly 18 "uses of examine()" '[.|\s]examine\(' -P # If this fails it's likely because you used '/atom/proc/examine(mob)' instead of '/proc/examinate(mob, atom)' - Exception: An examine()-proc may call other examine()-procs
