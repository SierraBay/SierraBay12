/datum/map/sierra
	shuttle_called_message = "Внимание всему персоналу: запущена процедура подготовки к подпространственному прыжку в %SECTOR%. Расчетное время окончания зарядки генератора блюспейса: %ETA%."
...
/datum/map/sierra/level_x_biohazard_announcement(bio_level)
	...
	switch(bio_level)
		...
		if(8)
			msg = "неопознанное биомеханическое заражение. Обнаружено несанкционированное подключение к энергосети"
		...
		if(7)
			...
			crew = "Инженерному и Охранному"
			sound = sound(ANNOUNCER_OUTBREAK7, volume = 100)
		if(6)
