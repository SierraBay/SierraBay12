...
/datum/map
	...
	/// The faction that the map is affiliated with, used for trade stuff.
	/// By default, it's set to the independent faction, which is the "neutral" faction.
	/// This can be changed by the map to determine which faction it should be affiliated with.
...
/datum/map/proc/setup_economy()
	for (var/datum/feed_network/N in news_network)
		N.CreateFeedChannel("Nyx Daily", "[company_short] Minister of Information", 1, 1) // [ERIS MODPACK EDIT]
